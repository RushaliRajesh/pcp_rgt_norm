...
def param2norm (pred_params , ori_params, init, ori_norms):
    ...
    print("theta",theta_ori)
    # print(params.shape)
    sin_theta_norm_ori = torch.sin(theta_ori).reshape(-1,1)
    axis_ori =torch.where(sin_theta_norm_ori > 0, sin_ori / (sin_ori.norm(dim=1).unsqueeze(1)), torch.tensor([[1.0, 0.0, 0.0]])) #checked
    print(axis_ori.shape)
    axis_angle_ori = axis_ori * theta_ori.unsqueeze(1) #checked
...
def train_pcpnet(opt):
    ...
    PATH = "mse_6_4params_g_lr_cor_norm.pt"
    ...
    for epoch in range(cont_epoch,opt.nepoch):
        train_loss = []
        val_loss = []
        model.train()
        ...
        if epoch % 10 == 0:
            EPOCH = epoch
            PATH = "mse_6_4params_g_lr_cor_norm_cont.pt"
            LOSS = tot_train_loss
